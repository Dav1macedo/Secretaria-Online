...
public class CadastroDisciplina extends JFrame implements ActionListener 
{
   JButton B1, B2;
   ...
   {
      setTitle("Tela de Cadastro dos Dados da Disciplina");
      setSize(500,500);
      setLocation(80,50);
      setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		
      L1 = new JLabel("CADASTRO DOS DADOS DO DISCIPLINA");
         ...
         if (e.getSource() == B1)
           ...
           private void btnCadastrarActionPerformed() {
        	    try {
        	        ...
        	        String nomeDisc = NomeDisc.getText();
        	        String profDisc = ProfDisc.getText();
        	        String cursoDisc = CursoDisc.getText();  

        	        ArmazenaDTO objarmazenadto = new ArmazenaDTO();
        	        objarmazenadto.setIdDisc(idDisc);
        	        objarmazenadto.setnomeDisc(nomeDisc);
        	        objarmazenadto.setProfDisc(profDisc);
        	        objarmazenadto.setcursoDisc(cursoDisc);
        	        


        	        InsereDao objinseredao = new InsereDao();
        	        objinseredao.InserirDados2(objarmazenadto); 

        	        JOptionPane.showMessageDialog(this, "Disciplina Cadastrada com sucesso!");
        	    } catch (Exception e) {
        	        JOptionPane.showMessageDialog(this, "Erro ao Cadastrado os dados: " + e.getMessage());
        	    }
        	}
