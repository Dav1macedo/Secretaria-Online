...
public class CadastroAluno extends JFrame implements  ActionListener, ListSelectionListener, ItemListener  
{
   JButton B1,B2;
   JTextField Nome, CPF, Ra, Est, Tel, Email;
   ...
   {
      setTitle("Tela de Cadastro dos Dados do Aluno");
      setSize(550,550);
      setLocation(80,50);
      setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		
      L1 = new JLabel("CADASTRO DOS DADOS DO ALUNO");
      ...
      Cur.addListSelectionListener(this);
...
private void btnCadastrarActionPerformed() {
    try {
        ...
        String cur = Cur.getSelectedValue();
        
      

        ArmazenaDTO objarmazenadto = new ArmazenaDTO();
        objarmazenadto.setRa(RA);
        objarmazenadto.setNome(nome);
        objarmazenadto.setCPF(cpf);
        objarmazenadto.setCur(cur);
        objarmazenadto.setEmail(email);
        objarmazenadto.setTel(tel);
        objarmazenadto.setEst(est);
        
        


        InsereDao objinseredao = new InsereDao();
        objinseredao.InserirDados(objarmazenadto); 
